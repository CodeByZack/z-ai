...
export function useSpeechRecognition(
  ...
  // Keep latest callback in a ref so the recognition instance always calls the current one
  ...
  // Shared cleanup: reset state and clear recognition ref
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      if (finalText) {
        // Always use the latest callback via ref
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if (!msg) {
        // no-speech / aborted — expected, still clean up state
        cleanup();
        return;
      }
    ...
    if (errorTimerRef.current) clearTimeout(errorTimerRef.current);
    setInterimText("");
  }, [isSupported, lang, setErrorAutoClear, cleanup]);
